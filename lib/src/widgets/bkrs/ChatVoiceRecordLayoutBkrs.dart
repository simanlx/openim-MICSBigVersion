...
import 'package:mics_big_version/src/widgets/bkrs/ChatRecordVoiceViewBkrs.dart';

typedef SpeakViewChildBuilder = Widget Function(ChatVoiceRecordBar recordBar);

class ChatVoiceRecordLayoutBkrs extends StatefulWidget {
  const ChatVoiceRecordLayoutBkrs({
    Key? key,
  ...
  @override
  _ChatVoiceRecordLayoutBkrsState createState() => _ChatVoiceRecordLayoutBkrsState();
}

class _ChatVoiceRecordLayoutBkrsState extends State<ChatVoiceRecordLayoutBkrs> {
  ...
  ChatVoiceRecordBar _createSpeakBar() => ChatVoiceRecordBar(
    onLongPressMoveUpdate: (details) {
      ...
      print("录音移动的位置x   ${global.dx}    y   ${global.dy}");
      setState(() {
        _selectedPressArea = global.dy >= 300.h;
        _selectedCancelArea = /*global.dy >= 563.h &&*/
        global.dy < 300.h && global.dx < 500.w;
        _selectedSoundToWordArea = global.dy < 300.h && global.dx >= 500.w;
      });
    },
