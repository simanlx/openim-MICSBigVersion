...
class IMUtil {
    ...
    switch (message.contentType) {
      ...
      case MessageType.at_text:
        ...
        try {
          ...
          if (isAtSelf == true) {
            var me = '@${StrRes.you}';
            content = text.replaceAll('@${OpenIM.iMManager.uid}', '');
            // 会话有[@你]前缀
            if (!isConversation) {
              content = '$me$content';
            }
  ...
  //   );
  // }
  static String getChatTimeline(int ms) {
    ...
    if (DateUtil.isYesterdayByMs(ms, locTimeMs)) {
      return '${languageCode == 'zh' ? '昨天' : 'Yesterday'} ${DateUtil.formatDateMs(ms, format: 'HH:mm')}';
    }

    if (DateUtil.isWeek(ms, locMs: locTimeMs)) {
      return '${DateUtil.getWeekdayByMs(ms, languageCode: languageCode)} ${DateUtil.formatDateMs(ms, format: 'HH:mm')}';
    }

    if (DateUtil.yearIsEqualByMs(ms, locTimeMs)) {
      return DateUtil.formatDateMs(ms, format: 'MM/dd HH:mm');
    }
  ...
  /// 通知解析
  static String? parseNotification(Message message) {
    String? text;
        ...
        case MessageType.memberInvitedNotification:
          {
            ...
            final a = notification.opUser!.nickname;
            final b = notification.invitedUserList!
                .map((e) => e.nickname!)
                .toList()
                .join('、');
        ...
        case MessageType.memberKickedNotification:
          {
            ...
            final a = notification.opUser!.nickname;
            final b = notification.kickedUserList!
                .map((e) => e.nickname)
                .toList()
                .join('、');
